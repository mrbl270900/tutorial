...
def get_task(data, alg, sent_tasks, tasks, low_low, med_low, high_low, low_med, med_med, high_med, low_high, med_high, high_high):
  if alg == "catagory":
    ...
    dwelltime = 25
    speed_score = data.speed * (dwelltime/8) # divide by 6 to give some liway to send task back and forth, for non stactic speeds and 
    #also for comm/procseing cost as they are not ran at same time
    link_speed_score = data.link_speed * (dwelltime/8)
    return_tasks = []
